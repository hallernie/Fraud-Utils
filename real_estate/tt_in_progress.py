...
# Regex for "unit_" file
unit_file_re = re.compile(r"unit_")
...
[
 ...
 ['3. The person who prepares an abstract of title for a parcel of real estate', 'A) insures the condition of the title.', 'B) inspects the property.', 'C) issues title insurance.', 'D) searches the public records and then summarizes the events and proceedings that affect title.', 'D', 'Unit 1: A description']
 
 ['4. When A recorded the deed received from B, the legal consequence of the recording was to', 'A) give B assurance of holding a first lien.', 'B) protect B from existing adverse claims.', 'C) transfer title.', 'D) serve as constructive notice of A’s interest.', 'D', 'Unit 10: A description']
...
##### method to read "unit" files in the current directory
#
def get_unit_file_list():
    file_list = []
    for file in os.listdir():
        if unit_file_re.match(file):
            file_list.append(file)
    
    return file_list
##### end: get_unit_file_list()


##### method to list the available units
##### also returns the available units in a set
#
def get_unit_file_set(questions):
    set_of_units = set([])
    for val in questions:
        set_of_units.add(val[6])

    return set_of_units

questions = []
for input_file in get_unit_file_list():
    questions.extend(get_questions_and_answers(input_file))

tmp = get_unit_file_set(questions)
for x,val in enumerate(sorted(tmp)):
    print(f"{x}:  {val}")
